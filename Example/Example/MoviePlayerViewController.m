...
//    }];
    ...
    [manager GET:@"http://xzb.qcloud.com/get_live_list" parameters:nil headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        [manager invalidateSessionCancelingTasks:YES resetSession:NO];
        if ([J2Num([responseObject valueForKeyPath:@"code"]) intValue] != 200) {
            [self hudMessage:@"直播列表请求失败"];
            return;
